...
def is_session_expired_error(exc: Exception | str) -> bool:
    text = str(exc or "").lower()
    markers = (
        "http 401",
        "error': '401",
        'error": "401',
        "text/html",
        "no ninja zenshin characters available",
    )
    return any(marker in text for marker in markers)
...
def relogin_and_reselect_character(char_id: Any) -> bool:
    ...
    try:
        ...
        amf_req.reset_web_session()
        login_result = amf_req.login(username, password, "", str(config.BUILD_NUM))
...
def handle_runtime_exception(
    ...
    if is_gateway_blocked_error(exc):
        ...
        print(
            "The official gateway is currently blocked by Cloudflare for direct AMF panel traffic."
        )
        print(
    ...
    if is_session_expired_error(exc):
        print(f"{context} session expired: {exc}")
        print("Attempting recovery: rebuild Zenshin web session + relogin...")
