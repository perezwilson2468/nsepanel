...
def bundled_base_path() -> str:
    # PyInstaller onefile extracts into _MEIPASS. Nuitka onefile keeps __file__
    # under the extracted runtime directory, so resolve bundled assets from here.
    if hasattr(sys, "_MEIPASS"):
        return sys._MEIPASS
    return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def writable_base_path() -> str:
    # Store runtime-generated files next to the exe/script, not inside bundled data.
...
def writable_path(relative_path: str) -> str:
    full_path = os.path.join(writable_base_path(), relative_path)
...
def save_to_json(data,filename):
    with open(writable_path(f"{filename}.json"), 'w', encoding='utf-8') as json_file:
...
def fetch_json_url(url: str, timeout: int = 20):
    ...
    if resp.status_code >= 400 or "text/html" in content_type or content_prefix.lstrip().startswith(b"<!DOCTYPE html"):
        ...
        if _is_rift_base_game() and resp.status_code == 403:
            raise ValueError(
                f"Ninja Rift library request was blocked with HTTP 403 on {gateway_label}"
            )
        raise ValueError(
            f"HTTP request failed with status {resp.status_code} while loading JSON from {url}"
        )
...
def send_amf_request(service, params):
    ...
    if resp.status_code >= 400 or "text/html" in content_type or content_prefix.lstrip().startswith(b"<!DOCTYPE html"):
        gateway_label = config.get_current_amf_profile().get("label", "Unknown Gateway")
        current_base_game_id = config.get_current_base_game()["id"]

        if current_base_game_id == "ninjasaga" and resp.status_code == 403:
            raise ValueError(
                f"NinjaSaga gateway is blocked by Cloudflare (HTTP 403 HTML) on {gateway_label}. "
                "Official server currently rejects direct AMF requests from this panel."
            )

        if config.get_current_base_game()["id"] == "rift" and resp.status_code == 403:
            raise ValueError(
                f"Ninja Rift gateway blocked the AMF request with HTTP 403 (Cloudflare/website protection) on {gateway_label}"
            )
        raise ValueError(
    ...
    try:
        ...
        gateway_label = config.get_current_amf_profile().get("label", "Unknown Gateway")
        raise ValueError(
            f"Failed to decode AMF response from {gateway_label}: {exc}"
        ) from exc
...
def download_resource(resource_name):
    ...
    with open(writable_path(f"data/{resource_name}.json"), "w", encoding="utf-8") as json_file:
...
def save_fight_data(data):
    with open(writable_path("fight_logs.txt"), 'a', encoding='utf-8') as file:
