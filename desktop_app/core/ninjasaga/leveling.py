...
RANK_NAME_BY_ID = {
    ...
    RANK_GENIN: "Genin",
    RANK_CHUNIN: "Chunin",
    RANK_CHUNIN_TALENTED: "Chunin Talented",
    RANK_JOUNIN: "Jounin",
    RANK_JOUNIN_TALENTED: "Jounin Talented",
    RANK_SPECIAL_JOUNIN: "Special Jounin",
    RANK_SPECIAL_JOUNIN_TALENTED: "Special Jounin Talented",
    RANK_TUTOR: "Tutor",
    RANK_TUTOR_SENIOR: "Tutor Senior",
}
...
def _resolve_char_rank_with_refresh(char_id: Any, fallback_data: dict[str, Any] | None = None):
    ...
    if isinstance(refreshed, dict):
        ...
        return _extract_char_rank(refreshed)
    return None
...
def _tp_daily_mission_update_consumed(response: Any) -> bool:
    if not isinstance(response, dict):
        return False
    ...
    return code == "104" or "already completed today" in text or "already complete today" in text
...
def _run_rank_exam(
    ...
    if exam_name.startswith("Special Jounin"):
    ...
    refreshed_snapshot = _refresh_character_snapshot(char_id)
    refreshed_rank = _extract_char_rank(refreshed_snapshot or getattr(config, "char_data", None))
    if exam_name.startswith("Special Jounin") and (refreshed_rank or -1) < target_rank:
        last_amf_call_at = _apply_special_jounin_class_selection(char_id, profile, last_amf_call_at)
        refreshed_snapshot = _refresh_character_snapshot(char_id)
        refreshed_rank = _extract_char_rank(refreshed_snapshot or getattr(config, "char_data", None))
        refreshed_control = _extract_char_control(refreshed_snapshot or getattr(config, "char_data", None))
        if (refreshed_rank or -1) < target_rank and (refreshed_control or 0) > 0:
            print(
                f"[Exam] {exam_name} class selection completed with control={refreshed_control}. "
            )
            refreshed_rank = target_rank
    if exam_name.startswith("Tutor") and (refreshed_rank or -1) < target_rank:
        last_amf_call_at = _apply_tutor_reward_claim(char_id, profile, last_amf_call_at)
        refreshed_snapshot = _refresh_character_snapshot(char_id)
        refreshed_rank = _extract_char_rank(refreshed_snapshot or getattr(config, "char_data", None))
...
def ninjasaga_leveling(loop_times=None, training_mode: str | None = None):
    ...
    success_streak = 0
    training_mode_cfg = {
        "tp": {"reward_key": "tp", "reward_value": 10, "required_rank": RANK_JOUNIN, "required_rank_name": "Jounin", "required_level": 40, "daily": True, "mission_ids": TP_TRAINING_MISSIONS},
    ...
    while True:
                ...
                if char_level >= JOUNIN_LEVEL_CAP and (char_rank or -1) < RANK_SPECIAL_JOUNIN:
                    ...
                    if not exam_passed and (char_rank or -1) < RANK_SPECIAL_JOUNIN:
                        ...
                        if refreshed_rank is not None:
                            char_rank = refreshed_rank
                    if exam_passed:
                        last_amf_call_at = _apply_special_jounin_class_selection(
                            char_id=char_id,
                            profile=profile,
                            last_amf_call_at=last_amf_call_at,
                        )
        ...
        if not _is_success_response(start_result):
            ...
            if cooldown > 0:
                ...
                if not _register_failure_and_maybe_circuit(failure_timestamps, profile, cycle):
                    break
                wait_sec = max(cooldown, start_retry_delay_seconds)
                wait_min = round(wait_sec / 60, 1)
                print(
                    f"[Cycle {cycle}] startMission cooldown/lock detected, waiting {wait_min} minutes before retry..."
                )
        ...
        try:
            ...
            return
        if not _is_success_response(update_result):
            if normalized_training_mode == "tp" and _tp_daily_mission_update_consumed(update_result):
                if pending_training_missions is not None and mission_id in pending_training_missions:
                    pending_training_missions = [mid for mid in pending_training_missions if mid != mission_id]
                print(f"[Cycle {cycle}] TP mission {mission_id} already completed today during updateCharacter. Trying the next TP daily mission...")
                continue
            success_streak = 0
            if not _register_failure_and_maybe_circuit(failure_timestamps, profile, cycle):
                break
        ...
        if isinstance(update_result, dict):
            ...
            if _is_zenshin_runtime() and last_observed_progress != (None, None) and current_progress == last_observed_progress:
                ...
                if no_progress_cycles >= 3:
                    print(
                        f"[Cycle {cycle}] Error: {game_label} progress is stagnant after multiple successful missions. "
                        "Stopping so we do not loop forever."
                    )
                    break
            else:
                no_progress_cycles = 0
