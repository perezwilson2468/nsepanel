...
def start_daily_battle(mission, char_flat, char_id, session_key):
    ...
    if _is_sage_scroll_mission(mission):
        ...
        print(f"Waiting 25 seconds for battle to settle...")
        time.sleep(25)
        return start_result
    ...
    print(f"Waiting {BATTLE_SETTLE_DELAY_SECONDS} seconds for battle to settle...")
    time.sleep(BATTLE_SETTLE_DELAY_SECONDS)
    return battle_id
    ...
    for mission_group, mission_id, run_count in mission_items:
        ...
        for attempt in range(run_count):
            ...
            if attempt < run_count - 1:
                # print(f"Waiting {POST_MISSION_DELAY_SECONDS} seconds before retrying {mission_id}...")
                time.sleep(POST_MISSION_DELAY_SECONDS)
        if not check_stop_event():
            # print(f"Waiting {POST_MISSION_DELAY_SECONDS} seconds before next mission...")
            time.sleep(POST_MISSION_DELAY_SECONDS)
