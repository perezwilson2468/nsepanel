...
def mission_s():
    ...
    while not check_stop_event():
        ...
        if stage_to_run is None:
            print(
                "Mission S has no unlocked stage available for the current level and energy"
            )
            return
        ...
        print(
            f"Running Mission S stage {stage_to_run}: {mission['name']} "
            f"(cost: {stage_cfg['energy_cost']})"
        )
        ...
        if isinstance(result, dict) and result.get("status") == 1:
            ...
            try:
                ...
                pass
            print(
                f"Mission S completed: {mission['name']} | "
                f"EXP: {xp}, Gold: {gold}, Level: {level}"
            )
        else:
