...
class MonsterHunt:
    def __init__(self):
        ...
        if not isinstance(self.char_data, dict):
        ...
        character_data = self.char_data.get("character_data") or self.char_data
        self.char_id = character_data.get("character_id") or character_data.get("char_id")
        if not self.char_id:
            raise ValueError("Character ID is not available for Monster Hunt")

        self.session_key = config.login_data.get("sessionkey") or config.login_data.get("session_key")
        if not self.session_key:
            raise ValueError("Session key is not available for Monster Hunt")
