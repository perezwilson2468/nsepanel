...
from ..core.utils import StatManager, fetch_json_url, flatten_json, send_amf_request
...
def fetch_bootstrap(force: bool = False):
    ...
    config.GATEWAY = current_profile.get("gateway")
    config.GAME_BUILD_NUM = current_profile.get("build_num")
    config.BUILD_NUM = config.GAME_BUILD_NUM
    return bootstrap
...
def check_version():
    ...
    version = bootstrap.get("version") or config.GAME_BUILD_NUM
...
def _verify_login_session(result: dict):
    ...
    verify_result = send_amf_request(_rift_amf_method("verifyFiles"), [session_key, verification_hash])
    ...
    cache_check_result = send_amf_request(_rift_amf_method("checkClearCache"), [account_id])
    result["check_clear_cache_result"] = cache_check_result
    if isinstance(cache_check_result, list) and cache_check_result:
        cache_cleared_result = send_amf_request(_rift_amf_method("setCacheCleared"), [account_id, cache_check_result])
...
def get_all_characters():
    ...
    result = send_amf_request(_rift_amf_method("getAllCharacters"), [account_id, session_key])
...
def get_character_data(char_id):
    session_key = config.login_data.get("sessionkey") or config.login_data.get("session_key")
    result = send_amf_request(_rift_amf_method("getCharacterData"), [char_id, session_key])
