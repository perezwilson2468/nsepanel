...
def _wait_seconds(seconds: int | float) -> bool:
    total = max(0.0, float(seconds))
    if total <= 0:
        return not check_stop_event()
...
def _record_failure(failure_timestamps: list[float]) -> bool:
    now = time.time()
    window_seconds = max(30, _cfg_int("leveling_failure_window_seconds", 180))
    max_failures = max(1, _cfg_int("leveling_max_failures_in_window", 6))
    failure_timestamps[:] = [ts for ts in failure_timestamps if now - ts <= window_seconds]
    failure_timestamps.append(now)
    if len(failure_timestamps) < max_failures:
        return True
    cooldown_seconds = max(10, _cfg_int("leveling_circuit_cooldown_seconds", 120))
    print(
        f"Failure safety triggered ({len(failure_timestamps)}/{max_failures} in {window_seconds}s). "
        f"Cooling down for {cooldown_seconds}s..."
    )
    failure_timestamps.clear()
    return _wait_seconds(cooldown_seconds)
    ...
    if isinstance(result, dict) and result.get("status") == 1:
        ...
        if level_gate == 60:
            print(
                ...
                f"{_cfg_text('sage_special_jounin_class_skill', DEFAULT_SPECIAL_JOUNIN_CLASS_SKILL)}."
            )
        return True
...
def run_supported_exam_stages(level_gate, char_id, session_key, exam_data):
    ...
    for progress_index in sorted(registry.keys()):
        ...
        exam_start_delay = _cfg_int("sage_exam_start_delay_seconds", EXAM_START_DELAY_SECONDS)
        print(f"Waiting {exam_start_delay} seconds before finishing exam stage...")
        time.sleep(exam_start_delay)
        ...
        exam_finish_delay = _cfg_int("sage_exam_finish_delay_seconds", EXAM_FINISH_DELAY_SECONDS)
        print(f"Waiting {exam_finish_delay} seconds after finishing exam stage...")
        time.sleep(exam_finish_delay)
        ran_any_stage = True
...
def process_mission(mission_same_level, char_level, char_id, session_key, wait_seconds=5):
    ...
    try:
        ...
        if result["status"] == 1:
            ...
            post_finish_delay = _cfg_int("sage_post_finish_delay_seconds", 0)
            if post_finish_delay > 0:
                print(f"Waiting {post_finish_delay} seconds after finish...")
                if not _wait_seconds(post_finish_delay):
                    return char_level, False
...
def start_leveling(loop_times=None):
    ...
    if loop_times is None:
        ...
        while True:
            ...
            infinite_rest_every = _cfg_int("sage_infinite_loop_rest_every_cycles", 50)
            infinite_rest_duration = _cfg_int("sage_infinite_loop_rest_duration_seconds", 10)
            if infinite_rest_every > 0 and iter_count == infinite_rest_every and iter_count != 0:
                print(f"Waiting {infinite_rest_duration} seconds after {infinite_rest_every} mission... ")
                
                # Check stop event during wait
                for i in range(infinite_rest_duration):
                    if check_stop_event():
                        break
                    time.sleep(1)
                else:
            ...
            new_level, success = process_mission(
                mission_same_level,
                char_level,
                char_id,
                session_key,
                wait_seconds=battle_wait_seconds,
            )
            ...
            if success:
                ...
                if not _record_failure(failure_timestamps):
                    break
        ...
        for i in range(loop_times):
            ...
            limited_rest_every = _cfg_int("sage_limited_loop_rest_every_cycles", 15)
            limited_rest_duration = _cfg_int("sage_limited_loop_rest_duration_seconds", 10)
            if limited_rest_every > 0 and i % limited_rest_every == 0 and i != 0:
                print(f"Rate limit reached. Waiting {limited_rest_duration} seconds...")
                
                # Check stop event during wait
                for wait_index in range(limited_rest_duration):
                    if check_stop_event():
                        break
                    time.sleep(1)
                else:
