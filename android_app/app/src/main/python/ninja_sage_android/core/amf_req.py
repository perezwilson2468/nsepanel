...
from .utils import send_amf_request, save_to_json, Crypt, CryptExact, get_random_n_seed
from . import config
...
def check_version():
    result = send_amf_request("SystemLogin.checkVersion", [config.GAME_BUILD_NUM])
    return result
    ...
    if result and "total_characters" in result and "account_data" in result:
        ...
        for i in range(total_chars):
            if i < len(account_data):
                ...
                print(f"{i + 1}. ID: {char_id}, Name: {char_name}, Level: {char_level}, Class: {char_class}")
...
def login(username, password, char_dot__, char_dot__underscore):
    ...
    def build_params(encrypted_password):
        return [
            username,
            encrypted_password,
    ...
    encrypted_default = Crypt.encrypt(password, char_dot__, char_dot__underscore)
    result = send_amf_request("SystemLogin.loginUser", build_params(encrypted_default))

    message = str(result.get("message", "")).lower() if isinstance(result, dict) else ""
    if isinstance(result, dict) and result.get("status") != 1 and "invalid password" in message:
        print("Default encryption rejected, retrying with exact encryption mode")
        encrypted_exact = CryptExact.encrypt(password, char_dot__, char_dot__underscore)
        result = send_amf_request("SystemLogin.loginUser", build_params(encrypted_exact))
